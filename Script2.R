...
Edgelist.List.Filtered <- lapply(1:length(Edgelist.List), function (x) {as.data.frame(ifelse (as.matrix(Filter.Incorporation[[x]]), as.matrix(Edgelist.List[[x]]), "NOTINC"))})
Edgelist.List.Filtered2 <- lapply((length(Edgelist.List)+1):length(Filter.Incorporation), function (x) {as.data.frame(ifelse (as.matrix(Filter.Incorporation[[x]]), as.matrix(Edgelist.List[[length(Edgelist.List)]]), "NOTINC"))})
...
Edgelist.List.Filtered[1:length(Filter.Discontinuation)] <- lapply(1:length(Filter.Discontinuation), function (x) {as.data.frame(ifelse (as.matrix(Filter.Discontinuation[[x]]), as.matrix(Edgelist.List.Filtered[[x]]), "DISCONT"))})
...
Edgelist.List.Filtered <- lapply(1:length(Edgelist.List.Filtered), function (x) dplyr::mutate_all(Edgelist.List.Filtered[[x]], list(~na_if(.,""))))
Edgelist.List.Filtered <- lapply(1:length(Edgelist.List.Filtered), function (x) Edgelist.List.Filtered[[x]][!apply(Edgelist.List.Filtered[[x]],1, function (y) {all(str_detect(y,"DISCONT|END|NOTINC") | is.na(y))}),])
...
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered), function (x) cbind(Edgelist.List.Filtered[[x]][0], Path=do.call(paste, c(Edgelist.List.Filtered[[x]], sep=","))))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) cSplit(Edgelist.List.Filtered.Temp[[x]], "Path", sep = "DISCONT", direction = "long"))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) cSplit(Edgelist.List.Filtered.Temp[[x]], "Path", sep = "NOTINC", direction = "long"))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) data.frame(str_split_fixed(data.frame(Edgelist.List.Filtered.Temp[[x]])[,1], "," ,15)))


## check for empty rows

Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) dplyr::mutate_all(Edgelist.List.Filtered.Temp[[x]], list(~na_if(.,""))))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) dplyr::mutate_all(Edgelist.List.Filtered.Temp[[x]], list(~na_if(.,"NA"))))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) dplyr::mutate_all(Edgelist.List.Filtered.Temp[[x]], list(~na_if(.,"END"))))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) dplyr::mutate_all(Edgelist.List.Filtered.Temp[[x]], list(~na_if(.,"APPEND")))) ## this line might seem odd, but i have manually controlled the Edgelists and the only company left with an "APPEND" marker is DE2350060811, which is a GUO company
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) Edgelist.List.Filtered.Temp[[x]][!apply(Edgelist.List.Filtered.Temp[[x]],1, function (y) {all  (is.na(y))}),])
...
## remove duplicates, to remove list elements that are entirely equal, we convert the data frames to characters and remove all characters that are 100% equal to each other, at this stage i started using "pblapply" because some tasks took a really long time
...
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) as.data.frame(Edgelist.List.Filtered.Temp[[x]]))
Edgelist.List.Filtered.Temp <- lapply(1:length(Edgelist.List.Filtered.Temp), function (x) setDT(Edgelist.List.Filtered.Temp[[x]]))
...
Edgelist.List.Filtered.Temp5 <- lapply(1:length(Edgelist.List.Filtered.Temp4), function (x) setDT(Edgelist.List.Filtered.Temp4[[x]]))
...
Edgelist.List.Filtered.Temp11 <- lapply(1:length(Edgelist.List.Filtered.Temp10), function(x) as.data.frame(Edgelist.List.Filtered.Temp10[[x]]))
Edgelist.List.Filtered.Temp11 <- lapply(1:length(Edgelist.List.Filtered.Temp11), function(x) Edgelist.List.Filtered.Temp11[[x]][!sapply(Edgelist.List.Filtered.Temp11[[x]], function(y) all(is.na(y)))])
Edgelist.List.Filtered.Temp11 <- lapply(1:length(Edgelist.List.Filtered.Temp11), function(x) setNames(Edgelist.List.Filtered.Temp11[[x]],c(paste0("X",1:ncol(Edgelist.List.Filtered.Temp11[[x]])))))
